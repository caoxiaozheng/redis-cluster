...
if [ "$1" = 'redis-cluster' ]; then
    ...
    if [ -z "$redis_password" ];then #Default to port 123456
    ...
    for port in $(seq $INITIAL_PORT $max_port); do
      ...
      if [ "$port" -lt "$first_standalone" ]; then
        PORT=${port} REDIS_PASSWORD=${redis_password} envsubst < /redis-conf/redis-cluster.tmpl > /redis-conf/${port}/redis.conf
        nodes="$nodes $IP:$port"
      else
        PORT=${port} REDIS_PASSWORD=${redis_password} envsubst < /redis-conf/redis.tmpl > /redis-conf/${port}/redis.conf
      fi
